 ...
 */
/* 현재 이 소스는 z좌표를 바탕으로 시간에 따라 회전하면서 점점 멀어지는 소스이다. view의 설정값에 대해 반대로(-1 곱한 형태)로 움직이는 것을 알 수 있다. 이러한 이유는 실질적으로 카메라가 움직이는 것이 아니라 화면이 변환 되는 것이므로 반대로 움직이는 것이다.
 */
#define GLEW_STATIC
...
int main(){
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        view  = glm::translate(view, glm::vec3(0.0f, 0.0f, -1 *timevalue));
        view  = glm::rotate(view, glm::radians(10 * timevalue), glm::vec3(0.0f, 0.0f, 1.0f));
