/* camera example4 : 오일러 각을 이용하여 마우스를 통해 카메라의 위치를 변환하는 코드 + 마우스 스크롤을 통한 줌 기능 구현을 확인해보는 코드
 */
#define GLEW_STATIC
...
//마우스 변환을 위한 셋팅
const float lastx = 400, lasty = 300;
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    // pitch에 제한을 둬서 이상한 카메라 뷰를 가지는 것에 제한사항을 둠
    if (pitch > 89.0f)
        pitch = 89.0f;
    if (pitch < -89.0f)
        pitch = -89.0f;
