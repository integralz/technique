 ...
 */
/* 하단 while 문 안에 변경해야할 부분에 대해 주석으로 표시하였다. FOV를 줄인다면 당연히 보이는 시야각이 좁아지고, ratio를 줄인다면 그에 따라 너비의 비중이 작아지면서 좌우로 넙데데한 뷰가 그려진다. 반대로 ratio가 커진다면 위아래로 넙데데한 뷰가 그려진다.
 */
#define GLEW_STATIC
...
int main(){
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        //1번의 해답은 여기서 첫번째, 두번째 파라미터를 변경하면서 실험을 진행해보면 된다.
